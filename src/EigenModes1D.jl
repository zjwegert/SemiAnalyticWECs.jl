  ...
  for j ∈ eachindex(μ)
    for k ∈ eachindex(x)
      ...
      elseif rem(j-1,2) == 0    # symmetric modes
        ...
        ∂ₓ⁴U[j,k] = μ[j]^4/sqrt(2*L)*(cos(μ[j]*x[k])/cos(μ[j]*L)+cosh(μ[j]*x[k])/cosh(μ[j]*L));
        ...
        ∂ₓ⁴U[j,k] =μ[j]^4/sqrt(2*L)*(sin(μ[j]*x[k])/sin(μ[j]*L)+sinh(μ[j]*x[k])/sinh(μ[j]*L));
      end
    end
  end
...
function _eigenmodes_clamped_clamped(N,L,x;debug)
  ...
  ∂ₓ⁴u(x,μ,α) = @. μ^4*(α[1]*exp(μ*x) + α[2]*exp(-μ*x) + α[3]*cos(μ*x) + α[4]*sin(μ*x));
